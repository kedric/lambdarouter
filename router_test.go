...
func serve(router *TreeMux, w http.ResponseWriter, r *http.Request, useLookup bool) bool {
	if useLookup {
		...
		result, found := router.Lookup(event)
		event.PathParameters = result.params
		router.ServeLookupResult(context.Background(), event, result)
		return found
	} else {
		router.ServeHTTP(w, r)
		return true
...
func testMethods(t *testing.T, newRequest RequestCreator, headCanUseGet bool, useSeparateLookup bool) {
	...
	testMethod := func(method, expect string) {
		...
		r, _ := newRequest(method, "/__stage__/user/"+method, nil)
...
func TestNotFound(t *testing.T) {
	...
	notFoundHandler := func(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
		calledNotFound = true
...
func TestMethodNotAllowedHandler(t *testing.T) {
	...
	notAllowedHandler := func(ctx context.Context, req events.APIGatewayProxyRequest, allow string) (events.APIGatewayProxyResponse, error) {

		calledNotAllowed = true

		expected := "GET PUT DELETE HEAD"
	...
	w := httptest.NewRecorder()
	r, _ := newRequest("POST", "/__stage__/user/abc", nil)
	router.ServeHTTP(w, r)
...
func TestOptionsHandler(t *testing.T) {
	optionsHandler := func(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	customOptionsHandler := func(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
		return events.APIGatewayProxyResponse{
			StatusCode: 401,
			Headers: map[string]string{
				"Access-Control-Allow-Origin": "httptreemux.com",
			},
		}, nil
	}
	...
	w := httptest.NewRecorder()
	r, _ := newRequest("OPTIONS", "/__stage__/user/abc", nil)
	router.ServeHTTP(w, r)
...
func testRedirect(t *testing.T, defaultBehavior, getBehavior, postBehavior RedirectBehavior, customMethods bool,
	...
	var redirHandler = func(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
		return events.APIGatewayProxyResponse{
			StatusCode: 301,
		}, nil
	}
	...
	for method, expectedCode := range expectedCodeMap {
		...
		w := httptest.NewRecorder()
		r, _ := newRequest(method, "/__stage__/slash", nil)
		...
		if expectedCode != http.StatusNoContent && w.Header().Get("Location") != "/__stage__/slash/" {
			t.Errorf("/slash was not redirected to /slash/")
		}

		r, _ = newRequest(method, "/__stage__/noslash/", nil)
		w = httptest.NewRecorder()
		found = serve(router, w, r, serveStyle)
		if found == false {
		...
		r, _ = newRequest(method, "/__stage__/noslash?a=1&b=2", nil)
		w = httptest.NewRecorder()
		found = serve(router, w, r, serveStyle)
		if found == false {
			t.Errorf("/noslash (non-redirect): found returned false")
		...
		r, _ = newRequest(method, "/__stage__/slash/?a=1&b=2", nil)
		w = httptest.NewRecorder()
		found = serve(router, w, r, serveStyle)
		if found == false {
...
func TestCatchAllTrailingSlashRedirect(t *testing.T) {
	...
	testPath := func(path string) {
		r, _ := newRequest("GET", "/__stage__/abc/"+path, nil)
		w := httptest.NewRecorder()
		router.ServeHTTP(w, r)
...
func TestRoot(t *testing.T) {
	for _, scenario := range scenarios {
		...
		r, _ := scenario.RequestCreator("GET", "/__stage__/", nil)
		w := new(mockResponseWriter)
...
func TestWildcardAtSplitNode(t *testing.T) {
	...
	simpleHandler := func(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
		t.Log(req.PathParameters)
		suppliedParam, _ = req.PathParameters["slug"]
		return events.APIGatewayProxyResponse{
			StatusCode: 200,
		}, nil
	}
	...
	r, _ := newRequest("GET", "/__stage__/patch", nil)
	w := httptest.NewRecorder()
	router.ServeHTTP(w, r)
	...
	suppliedParam = ""
	r, _ = newRequest("GET", "/__stage__/patch/abc", nil)
	w = httptest.NewRecorder()
	router.ServeHTTP(w, r)
	...
	r, _ = newRequest("GET", "/__stage__//patch/def", nil)
	w = httptest.NewRecorder()
	router.ServeHTTP(w, r)
...
func TestSlash(t *testing.T) {
	...
	handler := func(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
		param = req.PathParameters["param"]
		return events.APIGatewayProxyResponse{
			StatusCode: 204,
		}, nil
	}
	ymHandler := func(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
		param = req.PathParameters["year"] + " " + req.PathParameters["month"]
		return events.APIGatewayProxyResponse{
			StatusCode: 204,
		}, nil
	}
	...
	r, _ := newRequest("GET", "/__stage__/abc/de%2ff", nil)
	w := new(mockResponseWriter)
	router.ServeHTTP(w, r)
	...
	r, _ = newRequest("GET", "/__stage__/year/de%2f/month/fg%2f", nil)
	router.ServeHTTP(w, r)
...
func TestQueryString(t *testing.T) {
	for _, scenario := range scenarios {
		...
		r, _ := scenario.RequestCreator("GET", "/__stage__/static?abc=def&ghi=jkl", nil)
		w := new(mockResponseWriter)
		...
		r, _ = scenario.RequestCreator("GET", "/__stage__/wildcard/aaa?abc=def", nil)
		serve(router, w, r, scenario.ServeStyle)
		if param != "aaa" {
			t.Error("Expected wildcard to match aaa, saw", param)
		}

		r, _ = scenario.RequestCreator("GET", "/__stage__/catchall/bbb?abc=def", nil)
		serve(router, w, r, scenario.ServeStyle)
		if param != "bbb" {
...
func TestPathSource(t *testing.T) {
	...
	appleHandler := func(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
		called = "apples"
		return events.APIGatewayProxyResponse{
			StatusCode: 200,
		}, nil
	}
	bananaHandler := func(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
		called = "bananas"
		return events.APIGatewayProxyResponse{
			StatusCode: 200,
		}, nil
	...
	r, _ := newRequest("GET", "/__stage__/apples", nil)
	r.RequestURI = "/__stage__/bananas"
	w := new(mockResponseWriter)
